trigger UD_beforeDelete on UserData__c (before delete) {
	...
	for (UserData__c ud : trigger.old) {
		setCount = (ud.UserNumber__c / 60).intValue();
		InUseRoster__c thisRoster = userPattern.get(setCount);
		if (thisRoster != null) {
			thisRoster.Bitz__c = OAE_Utils.flipOff(thisRoster.Bitz__c.longValue(), 1L<<(Math.mod((ud.UserNumber__c-1).intValue(),60)), false);
		}
	}
